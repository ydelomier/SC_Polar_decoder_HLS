...
/*********************************************************************************/
	...
	void do_action(){
		...
		TYPE_LLRS reg_result;
		// stack
		sc_biguint<DEPTH_DIV * 2> stack = 0;
		sc_biguint< 2 > G_stack_value = 1;
		// Node type stack
		sc_biguint<DEPTH_DIV * 8> Node_type_stack = 0;
...
#ifdef DEBUG_LIGHT
cout << "	+++++ [F_STATE]  N_value : " << N << ", iteration : "<< new_N << ", adr_a : "<< adr_a << ", adr_b : "<< adr_b << ", w_a : "<< adr_w_a << ", w_b : "<< adr_w_b << endl;
#endif
...
cout << "					rs =  "; SHOW_LLRS<PAR, LLR_BITS >(result); cout << endl;
			...
			/**************************************************************/
				...
				R_STATE:
				{
					...
					TYPE_LLRS la = reg_result;

					TYPE_BITS ps;
					TYPE_BITS ref;

#ifdef DEBUG_LIGHT
cout << "	_____ [R_STATE]  N_value : " << N << ", Node : " << Node << ", FB : "<< Is_Frozen << ", adr_s : " << adr_s <<endl;
#endif
					...
					switch( type ){    // Node = 0x00 : R0, 0x0F : R1, 0x02 : REP, 0x03 : REP2, 0x04 : SPC, 0x05 : SPC2, 0x08 : ?
						...
						case 0x00 :
							ps = Spec_Node_R0<PAR, LLR_BITS> (la); break;
	#if ELAG_R1 == 1
						case 0x07 :
							ps = Spec_Node_R1<PAR, LLR_BITS> (la); break;
	#endif
	#if  ELAG_REP == 1
						case 0x01 :
		#if ELAG_REP2 == 0
							ps = Spec_REP_Node<PAR, LLR_BITS> (la); break;
		#else
							ps = Spec_REP_REP2_Node<PAR, LLR_BITS> (la, (bool)sel); break;
		#endif
	#endif
	#if  ELAG_SPC == 1
						case 0x02 :
		#if ELAG_SPC2 == 0
							ps = Spec_SPC_Node<PAR, LLR_BITS> (la); break;
		#else
							ps = Spec_SPC_SPC2_Node<PAR, LLR_BITS> (la, (bool)sel); break;
		#endif
	#endif
						default :
							ps = Spec_Polar_Decoder< PAR, LLR_BITS > (la, Is_Frozen);break;
					}
#else
					ps = Spec_Polar_Decoder< PAR, LLR_BITS > (la, Is_Frozen);
#endif
...
#ifdef _MONITORING_
					N_value.write( (int)(N<<1) );
					Fct_ID.write(0x48);
#endif
					stack = write_stack< 2, DEPTH_DIV >( stack, G_stack_value); // Stack management

#ifdef DEBUG_LIGHT
cout << "	같같� [G_STATE]  N_value : " << (N<<1) << ", iteration : "<< N  << ", adr_a : "<< adr_a << ", adr_b : "<< adr_b << ", ps_adr : " << ps_adr << ", w_a : "<< adr_w_a << ", w_b : "<< adr_w_b  << endl;
#endif
...
#ifdef DEBUG_LIGHT
cout << "	----- [H_STATE]  N_value : " << (N<<1) << ", iteration : "<< N << ", ps_adr_a : " <<ps_adr_a << ", ps_adr_b : " <<ps_adr_b << endl;
#endif
					////
					h_loop: for( i = 0; i < N ; i++)
					{
...
#ifdef DEBUG_LIGHT
cout << "	::::: [H0_STAT]  N_value : " << (N<<1) << ", iteration : "<< N << ", ps_adr_a : " <<ps_adr_a << ", ps_adr_b : " <<ps_adr_b << endl;
#endif
					////
					h0_loop: for( i = 0; i < N ; i++)
					{
					#pragma HLS DEPENDENCE variable=bit_mem_1 array inter RAW false
					#pragma HLS DEPENDENCE variable=bit_mem_2 array inter RAW false
					#pragma HLS PIPELINE
...
cout << "					sb =  "; SHOW_BITS<PAR >(sb); cout << endl;
			...
			/**************************************************************/
				...
				F_R0_STATE :
				{
					...
					/////////////////////////////
#ifdef DEBUG_LIGHT
cout << "	===== [F_ELAG_] ( R0 ) N_value : " << N << ", iteration : "<< new_N << ", adr_a : "<< adr_a << ", adr_b : "<< adr_b << ", adr_s : "<< adr_s   <<  endl;
#endif

					f_pruned_R0: for( i = 0; i < new_N ; i++)
					{
					#pragma HLS DEPENDENCE variable=llr_mem_b array inter RAW false
					#pragma HLS DEPENDENCE variable=llr_mem_a array inter RAW false
					#pragma HLS PIPELINE
...
cout << "					ps =  "; SHOW_BITS<PAR>(ps); cout << endl;
			...
			/**************************************************************/
				...
				F_R1_STATE :
				{
					...
					/////////////////////////////
#ifdef DEBUG_LIGHT
cout << "	===== [F_ELAG_] ( R1 ) N_value : " << N << ", iteration : "<< new_N << ", adr_a : "<< adr_a << ", adr_b : "<< adr_b << ", adr_s : "<< adr_s   <<  endl;
#endif

					f_pruned_R1: for( i = 0; i < new_N ; i++)
					{
					#pragma HLS DEPENDENCE variable=llr_mem_b array inter RAW false
					#pragma HLS DEPENDENCE variable=llr_mem_a array inter RAW false
					#pragma HLS PIPELINE
...
cout << "					ps =  "; SHOW_BITS<PAR>(ps); cout << endl;
			...
			/**************************************************************/
				...
				F_REP_STATE :
				{
					...
					/////////////////////////////
#ifdef DEBUG_LIGHT
cout << "	===== [F_ELAG_] (REP ) N_value : " << N << ", iteration : "<< new_N << ", adr_a : "<< adr_a << ", adr_b : "<< adr_b << ", adr_s : "<< adr_s   <<  endl;
#endif
					sc_bigint< LLR_BITS + LOG2_PAR + 1 > sum = 0;
					f_pruned_REP: for( i = 0; i < new_N ; i++)
					{
					#pragma HLS DEPENDENCE variable=llr_mem_b array inter RAW false
					#pragma HLS DEPENDENCE variable=llr_mem_a array inter RAW false
					#pragma HLS PIPELINE
...
cout << "					ps =  "; SHOW_BITS<PAR>(ps); cout << endl;
			...
			/**************************************************************/
				...
				F_SPC_STATE :
				{
					...
					/////////////////////////////
#ifdef DEBUG_LIGHT
cout << "	===== [F_ELAG_] (SPC ) N_value : " << N << ", iteration : "<< new_N << ", adr_a : "<< adr_a << ", adr_b : "<< adr_b << ", adr_s : "<< adr_s   <<  endl;
#endif
					...
					COUNTER adr_min = 0;
					TYPE_BITS sign_min;
					f_pruned_SPC: for( i = 0; i < new_N ; i++)
					{
					#pragma HLS DEPENDENCE variable=llr_mem_b array inter RAW false
					#pragma HLS DEPENDENCE variable=llr_mem_a array inter RAW false
					#pragma HLS PIPELINE
...
cout << "					ps     =  "; SHOW_BITS<PAR>(sign); cout << endl;
					...
					if( parity != 0 )
					{
						...
						bit_mem_1[ adr_s - new_N + adr_min ] = value;
						bit_mem_2[ adr_s - new_N + adr_min ] = value;
#ifdef DEBUG
cout << "	===== [F_ELAG_] Node SPC " << endl;
...
cout << "					adr    =  "<< adr_min << endl;
			...
			/**************************************************************/
				...
				G_R0_STATE :
				{
					...
					/////////////////////////////
#ifdef DEBUG_LIGHT
cout << "	##### [G_ELAG_] ( R0 ) N_value : " << (N<<1) << ", iteration : "<< N << ", adr_a : "<< adr_a << ", adr_b : "<< adr_b << ", ps_adr : "<< ps_adr << ", adr_s : "<< adr_s  << endl;
#endif

					g_pruned_R0: for( i = 0; i < N ; i++)
					{
					#pragma HLS DEPENDENCE variable=llr_mem_b array inter RAW false
					#pragma HLS DEPENDENCE variable=llr_mem_a array inter RAW false
					#pragma HLS PIPELINE
...
cout << "					ps =  "; SHOW_BITS<PAR>(ps); cout << endl;
			...
			/**************************************************************/
				...
				G_R1_STATE :
				{
					...
					/////////////////////////////
#ifdef DEBUG_LIGHT
cout << "	##### [G_ELAG_] ( R1 ) N_value : " << (N<<1) << ", iteration : "<< N << ", adr_a : "<< adr_a << ", adr_b : "<< adr_b << ", ps_adr : "<< ps_adr << ", adr_s : "<< adr_s  << endl;
#endif

					g_pruned_R1: for( i = 0; i < N ; i++)
					{
					#pragma HLS DEPENDENCE variable=llr_mem_b array inter RAW false
					#pragma HLS DEPENDENCE variable=llr_mem_a array inter RAW false
					#pragma HLS DEPENDENCE variable=bit_mem_1 array inter RAW false
...
cout << "					ps =  "; SHOW_BITS<PAR>(ps); cout << endl;
			...
			/**************************************************************/
				...
				G_REP_STATE :
				{
					...
					/////////////////////////////
#ifdef DEBUG_LIGHT
cout << "	##### [G_ELAG_] (REP ) N_value : " << (N<<1) << ", iteration : "<< N << ", adr_a : "<< adr_a << ", adr_b : "<< adr_b << ", ps_adr : "<< ps_adr << ", adr_s : "<< adr_s  << endl;
#endif

					sc_bigint< LLR_BITS + LOG2_PAR + 1 > sum = 0;
					g_pruned_REP: for( i = 0; i < N ; i++)
					{
					#pragma HLS DEPENDENCE variable=llr_mem_b array inter RAW false
					#pragma HLS DEPENDENCE variable=llr_mem_a array inter RAW false
					#pragma HLS DEPENDENCE variable=bit_mem_1 array inter RAW false
...
cout << "					ps =  "; SHOW_BITS<PAR>(ps); cout << endl;
			...
			/**************************************************************/
				...
				G_SPC_STATE :
				{
					...
					/////////////////////////////
#ifdef DEBUG_LIGHT
cout << "	##### [G_ELAG_] (SPC ) N_value : " << (N<<1) << ", iteration : "<< N << ", adr_a : "<< adr_a << ", adr_b : "<< adr_b << ", ps_adr : "<< ps_adr << ", adr_s : "<< adr_s  << endl;
#endif
...
cout << "					ps     =  "; SHOW_BITS<PAR>(sign); cout << endl;
					...
					{
						TYPE_BITS value = VECTOR_XOR< PAR >( sign_min, old_mask );  // Invert the min sign if parity check fail
						bit_mem_1[ adr_s - N + adr_min ] = value;
						bit_mem_2[ adr_s - N + adr_min ] = value;
#ifdef DEBUG
cout << "	##### [G_ELAG_] Node SPC " << endl;
cout << "					s_min  =  "; SHOW_BITS<PAR>(sign_min); cout << endl;
cout << "					o_mask =  "; SHOW_BITS<PAR>(old_mask); cout << endl;
